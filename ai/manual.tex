...
The generator is a Python script (\texttt{sm-builder.py}) that validates your model and produces Rust code.
...
\textbf{Example Usage of \texttt{in\_state}:}
Inside a guard or action, you can check peer states:
\begin{lstlisting}[language=Rust]
guard: ctx.in_state_heater_on() && ctx.temperature > 50.0
\end{lstlisting}

\section{Writing Code in Hooks \& Actions}
All logic hooks (\texttt{entry}, \texttt{do}, \texttt{exit}, \texttt{action}) allow you to write Rust code directly.
...
\toprule
\textbf{Variable} & \textbf{Type} & \textbf{Availability} & \textbf{Description} \\
\midrule
\endhead
...
\texttt{t\_src} & \texttt{\&str} & Action / Hook & The full path of the state being left. \\
\texttt{t\_dst} & \texttt{\&str} & Action / Hook & The full path of the target state. \\
\bottomrule
\end{longtable}

\textit{Note: \texttt{t\_dst} is not available if the transition targets a Decision Tree, as the final destination is not yet known.}

\section{Transitions}
A transition determines how the machine moves.

\subsection{Transition Flow}
When a transition fires:
\begin{enumerate}
    \item \textbf{Guard Check:} Is the condition true?
    \item \textbf{Action Execution:} Run the \texttt{action} code.
    \item \textbf{Exit Chain:} Run \texttt{exit} hooks for source state and parents.
    \item \textbf{Entry Chain:} Run \texttt{entry} hooks for target hierarchy.
    \item \textbf{Initialization:} Run \texttt{initial} transitions for composite targets.
\end{enumerate}

\subsection{Using Actions (The Mealy Aspect)}
The \texttt{action} keyword allows you to execute code \textit{during} the transition, before the old state is exited. This is ideal for logging, sending messages, or arithmetic that depends on the transition event itself.
...
states:
  reading:
    transitions:
      - guard: ctx.value > 100
        action: |
          println!("Alert: Moving from {} to {}", t_src, t_dst);
